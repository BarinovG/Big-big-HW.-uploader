...
class Yandex_user():
    ...
    #Получим список загруженных фото на ЯД для составления JSON. В параметрах папка, в которую загружали фото
    ...
    #Создадим джейсон по имени и размеру загруженных файлов. Информацию о загруженных файлах получаем с Яндекса, и вытаскиваем от туда их имена.
    #Информацию о размерах, обозначенных буквами, фото вытаскиваем из ВК
    def create_json(self, photo_name, photo_size):
        ...
        for name in photo_name:
            ...
            with open("data_file.json", "a") as f:
                json.dump(data, f)
                print(f"Информация о загрузке {name['name']} добавлена в json")